...
function Auth () {
  ...
  const getPermanentCode = () => {
    const temporalCode = new URLSearchParams(window.location.search).get(
      'code'
    )
    ...
    if (process.env.REACT_APP_ENV_MODE === 'production') {
      requestUrl =
        'https://open-telegram-to-notion-bot-backend.francisco-pessano.com/api/v1/auth'
    } else {
                ...
                <>
                  {permanentCode === 'empty'
                    ...
                    : (
                      <div className='error'>
                        <h3>
                  There has been an error getting the code. Please try again
                  later.
                      </h3>
                      </div>
                      )}
